...
onMounted(async() => {
    //@ts-ignore
    console.log("dsjkdsjksdjksd",dataO)
   //@ts-ignore
    let my_data = JSON.parse(dataO)
    console.log("zidata racho",my_data)
    let searchResult = await adminStore.getSearchedProperties(my_data).then((data) => {
        properties.value = data?.data?.properties
        number_of_records.value = data?.data?.total
        search_loader.value = false
        toast.add({ severity: 'contrast', summary: 'Search Complete', detail: 'Your filtered search is complete', life: 2000 });
    ...
    let subbs = await adminStore.getSuburbTotals(my_data).then((data) => {
