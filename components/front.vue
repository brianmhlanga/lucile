<template>
    <div style="page-break-after: auto; padding: 0;" class="container invoice-4 invoice-content mb-4">
        <div class="col-lg-12">
            <div style="min-width: 800px; height: 1100px;" class="invoice-inner position-relative" :class="{ 'shadow': shadow }" id="invoice_wrapper">
                <div class="invoice-top">
                    <div class="row">
                        ...
                        <div class="col-sm-6">
                            <div class="invoice text-end mt-4">
                                <h1>Invoice</h1>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="invoice-number">
                                <h3><strong>Invoice Number:</strong> #{{ invoice_number }}</h3>
                            </div>
                        </div>
                        <div class="col-sm-6 text-end">
                            <div class="invoice-date">
                                <h3><strong>Invoice Date:</strong> {{ date_issued }}</h3>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="invoice-info">
                    <div class="row">
                        <div class="col-sm-6 mb-30">
                            <div class="invoice-number">
                                <h4 class="inv-title-1"><strong>Bill To</strong></h4>
                                <p v-if="customer_contact_person" class="invo-addr-1">
                                    {{ customer_name }}<br>
                                    {{ customer_contact_person }}<br>
                                    {{ customer_email }}<br>
                                    {{ customer_address }}<br>
                                </p>
                                <p v-else class="invo-addr-1">
                                    {{ customer_name }}<br>
                                    {{ customer_email }}<br>
                                    {{ customer_address }}<br>
                                </p>
                        ...
                        <div class="col-sm-6 mb-30">
                            <div class="invoice-number text-end">
                                <h4 class="inv-title-1"><strong>Invoice By</strong></h4>
                                <p class="invo-addr-1">
                                    {{ company_name }}  <br>
                                    {{ company_email }} <br>
                                    {{ company_address }} <br>
                    ...
                    <div class="row">
                        <div class="col-sm-6 mb-30">
                            <h4 class="inv-title-1"><strong>Date</strong></h4>
                            <p class="inv-from-1">Due Date: {{ payment_due }}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="table-responsive">
                        <table class="table invoice-table">
                            ...
                            <tbody>
                                ...
                                <tr v-if="all_items.length <= max_front_page - table_row_offset">
                                    <td colspan="4" class="text-end">Sub Total</td>
                                    <td class="text-right">{{ (calculateGrandTotal()).toFixed(2) }}</td>
                                </tr>
                                <tr v-if="(all_items.length <= max_front_page - table_row_offset) && tax">
                ...
                <div v-if="all_items.length <= max_front_page - table_row_offset" class="invoice-informeshon mt-4">
                    <div class="row">
...
<script setup lang="ts">
    ...
    const { logo, invoice_number, customer_name, customer_contact_person, customer_email, invoice_items, customer_address, currency, _discount, _quantity, tax, company_name, company_email, company_address, company_website, due_date, payment_information, terms_n_conditions, company_phone_number, date_issued, payment_due } = data;
